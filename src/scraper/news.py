...
class StockSourcer:
    ...
    def get_company_news(self, ticker: str, days_back: int = 7):
        ...
        cols = ['datetime', 'headline', 'sentiment_score', 'mentioned_orgs', 'financial_figures', 'source', 'url']
    ...
    def get_executive_summary(self, df: pd.DataFrame):
        ...
        for ticker in df['ticker'].unique():
            ...
            label = "🟢 BULL" if top_story['sentiment_score'] > 0.1 else "🔴 BEAR" if top_story['sentiment_score'] < -0.1 else "⚪ NEUT"
            figures = f" | Data found: {top_story['financial_figures']}" if top_story['financial_figures'] else ""
